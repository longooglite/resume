...
export const useResumeData = () => {
  ...
  // Convenience slices
  const experiences = computed<WorkExperience[]>(() => resume.value.work_experience ?? [])
  const education = computed(() => resume.value.education ?? [])
  const technicalSkills = computed<RatedSkill[]>(() => resume.value.skills?.technical ?? [])
  const leadershipSkills = computed<RatedSkill[]>(() => resume.value.skills?.leadership_and_management ?? [])
  ...
  const toDate = (ymOrPresent: string): Date => {
    ...
    const iso = /^\d{4}-\d{2}$/.test(ymOrPresent) ? `${ymOrPresent}-01` : `${ymOrPresent}-01-01`
    return new Date(iso)
  }
  ...
  const currentRole = computed<WorkExperience | undefined>(() => {
    return experiences.value.find(e => e.end_date.toLowerCase?.() === 'present') ?? experiencesSorted.value[0]
  })
  ...
  const getExperienceByCompany = (company: string) =>
    experiences.value.filter(e => e.company.toLowerCase() === company.toLowerCase())
  ...
  const getSkillLevel = (name: string) =>
    technicalSkills.value.find(s => s.name.toLowerCase() === name.toLowerCase())?.level

  const companies = computed(() => Array.from(new Set(experiences.value.map(e => e.company))))
